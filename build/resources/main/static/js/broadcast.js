...
client.on("message", function (topic, payload) {
    if (topic == 'X/VR') {
        var version = getVersion(payload);
        console.log("主题:" + topic + ",消息:" + payload.toString());
        // alert("当前版本为:" + version);
        $("#versionNo").html("当前版本为: " + version);
    }
});

$(function () {
    $("#submitControl").click(commitFeedback);
    $("#parameterPanel").on("show.bs.modal", function () {
        $.ajax({
    ...
    $("#submit-param-confirm").click(dispatchParameters);
});
